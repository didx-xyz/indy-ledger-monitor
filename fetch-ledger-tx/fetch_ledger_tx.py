...
async def fetch_status(genesis_path: str, nodes: str = None, ident: DidKey = None, status_only: bool = False, alerts_only: bool = False):
    ...
    # txn = await get_pool_txns(pool)
    # print(txn)

    # print('XXXXXXXXXXXXXXXXXXXXX\n\n\n\n')

    # fetch one DOMAIN ledger transaction by specifying seq_no
    # txn = await get_txn(pool, 11)
    # print(json.dumps(txn, indent=2))

    # fetch range of DOMAIN ledger transactions by specifying seq_nos
    txn = await get_txn_range(pool, list(range(1, 10)))
    print(json.dumps(txn, indent=2))
